...
For this portfolio implementation, keep Ollama's typography, spacing restraint, and pill geometry as the primary visual reference, but preserve the site's existing two-brand accent system as a controlled exception:

- **Warm Brown**: `#7f534a` / `#a35243`
- **Slate Blue**: `#4a6170` / `#4f6c72`

Use these two accents sparingly and consistently:

- Warm Brown: research emphasis, selected state, warm secondary highlights
- Slate Blue: design/code emphasis, links, focus, interactive emphasis
- Accent surfaces should remain soft, but stronger than pure Ollama grayscale fills when placed on warm-white cards
- Badge/chip fills can use roughly `0.14–0.18` opacity and matching `0.22–0.24` border opacity so labels remain legible against the portfolio's layered paper background
...
### Portfolio Accent Exception
- **Research / Warm Accent** (`#7f534a`, strong `#a35243`): Reserved for research labels, selected emphasis, and warm metadata accents.
- **Design-Code / Cool Accent** (`#4a6170`, strong `#4f6c72`): Reserved for design/code labels, links, focus, and interactive emphasis.
- **Warm Accent Surface**: use a soft tint around `rgba(127, 83, 74, 0.14-0.18)` depending on contrast needs.
- **Cool Accent Surface**: use a soft tint around `rgba(74, 97, 112, 0.14-0.18)` depending on contrast needs.
- **Neutral Chip Surface**: when chips sit on warm-white cards, neutral fills can be slightly stronger than Ollama default, around `rgba(97, 93, 89, 0.08)`.
...
**Portfolio header adjustment**: avoid redundant fixed utility controls such as an always-visible `TOP` button in the header. Keep the right side limited to meaningful navigation or the mobile-menu trigger.
...
- For this portfolio only: preserve the warm-brown and slate-blue brand accents as the sole allowed chromatic exceptions
- When accent chips sit on warm-white cards, raise fill opacity slightly instead of changing layout or inventing new colors
...
- Don't add extra accent colors beyond the existing warm brown and slate blue pair
...
- Portfolio Warm Accent: "#7f534a" / strong "#a35243"
- Portfolio Cool Accent: "#4a6170" / strong "#4f6c72"
